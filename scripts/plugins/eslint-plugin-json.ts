...
export const fetchEslintPluginJsonRules = async (
	createRule: CreateRule,
): Promise<{ name: string; rules: Rule[] }> => {
	const rules: Rule[] = [];

	const response = await fetch(
		"https://raw.githubusercontent.com/eslint/json/refs/heads/main/README.md",
	);
		...
		.forEach((e) => {
			...
			rules.push(
				createRule(
					"eslintJson",
					name,
					`https://github.com/eslint/json/blob/main/${path}`,
				),
			);
		});
