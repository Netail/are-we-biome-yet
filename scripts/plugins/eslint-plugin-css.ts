...
export const fetchEslintPluginCssRules = async (
	createRule: CreateRule,
): Promise<{ name: string; rules: Rule[] }> => {
	const rules: Rule[] = [];

	const response = await fetch(
		"https://raw.githubusercontent.com/eslint/css/refs/heads/main/README.md",
	);
		...
		.forEach((e) => {
			...
			rules.push(
				createRule(
					"eslintCss",
					name,
					`https://github.com/eslint/css/blob/main/${path}`,
				),
			);
		});
