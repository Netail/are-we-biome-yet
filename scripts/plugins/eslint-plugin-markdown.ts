...
export const fetchEslintPluginMarkdownRules = async (
	createRule: CreateRule,
): Promise<{ name: string; rules: Rule[] }> => {
	const rules: Rule[] = [];

	const response = await fetch(
		"https://raw.githubusercontent.com/eslint/markdown/refs/heads/main/README.md",
	);
		...
		.forEach((e) => {
			...
			rules.push(
				createRule(
					"eslintMarkdown",
					name,
					`https://github.com/eslint/markdown/blob/main/${path}`,
				),
			);
		});
